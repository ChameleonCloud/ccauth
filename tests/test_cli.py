...
def test_build_sites_stamps_auth_config_on_ref_api_sites(monkeypatch):
    """Reference API sites get client_id and discovery_endpoint stamped from args."""
    # Return bare sites (as from_reference_api now does)
...
def test_enrich_skips_when_no_token(tmp_path, monkeypatch):
    ...
    mock_list = MagicMock()
    monkeypatch.setattr("ccauth.cli._list_projects_at", mock_list)
